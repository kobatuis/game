...
let groundY = 1000; // ここでピクセル指定を行います
...
function setObstaclePosition() {
    ...
    obstacles.forEach((obstacle, index) => {
        ...
        let attempts = 0; // 最大試行回数を設定
        while (!validPosition && attempts < 100) { // 100回試行しても配置できない場合はデフォルト位置に設定
            obstacleX = Math.random() * (screenWidth - obstacleWidth);
            validPosition = true;

            // キャラクターとの距離をチェック
            if (Math.abs(obstacleX - characterX) < 150) {
                validPosition = false;
                attempts++;
                continue;
            }
        ...
        // 最大試行回数を超えた場合はデフォルト位置に設定
        if (!validPosition) {
            console.warn(`Obstacle ${index + 1} could not be placed. Setting to default position.`);
            obstacleX = Math.random() * (screenWidth - obstacleWidth); // デフォルト位置を設定
        }
...
// キャラクターの位置とサイズをログ出力する関数
function logCharacterPositionAndSize() {
    ...
    console.log(`Character: Top = ${characterTop}, Bottom = ${characterBottom}, Height = ${characterHeight}`);
}
...
startButton.addEventListener('click', () => {
    if (!startFlag && countdownTimer === null) {
        ...
        startFlag = true; // ゲーム開始フラグをtrueに設定
        countdownNumber = 3;
        startVideo(); // Webカメラを起動
        update(); // ゲームの更新を開始
    }
});
...
window.addEventListener('resize', () => {
    ...
    logCharacterPositionAndSize(); // 統一した関数を呼び出す
});
...
async function update() {
    ...
    if (collision({ x: characterX, y: character.offsetTop, width: characterWidth, height: character.offsetHeight }, 
        ...
        if (goalTime === null) {
            ...
            obstacles.forEach(obstacle => {
                obstacle.style.display = 'none'; // 障害物を非表示に設定
            });
        }
    }
    ...
    logCharacterPositionAndSize(); // キャラクターの位置とサイズをログ出力
